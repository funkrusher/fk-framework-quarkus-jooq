...
import static org.fk.framework.dao.AbstractDAO.InsertType.*;
...
public abstract class AbstractDAO<R extends UpdatableRecord<R>, T> {
    ...
    protected AbstractDAO(final DSLContext dsl, final Table<R> table) {
        this.dsl = dsl;
        this.table = table;
        this.request = (RequestContext) dsl.data(RequestContext.DSL_DATA_KEY);
