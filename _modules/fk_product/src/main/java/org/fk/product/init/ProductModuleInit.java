...
import org.fk.root.init.RootModuleInit;
...
@Startup
@ApplicationScoped
public class ProductModuleInit {
    private static final String PRODUCT_MODULE_ID = "product";
    public static final Logger LOGGER = Logger.getLogger(ProductModuleInit.class);
    @Inject
    ProductModuleInit(
            final ModuleInitializer moduleInitializer,
            final Database1ConfigurationFactory configurationFactory,
            @SuppressWarnings("java:S1172") final RootModuleInit rootModuleInit) {

        LOGGER.info("ProductModuleInit ...");

        final DSLContext dsl = DSL.using(configurationFactory.getConfiguration());
        moduleInitializer.init(dsl, PRODUCT_MODULE_ID, Testshop.TESTSHOP.getQualifiedName(), tx1 -> {
