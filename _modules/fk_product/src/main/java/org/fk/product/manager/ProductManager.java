...
public class ProductManager extends AbstractManager {
    ...
    public String query(RequestContext requestContext, final FkQuery fkQuery) throws InvalidDataException {
        return database1.dsl(requestContext).transactionResult(tsx -> {
            final ProductRepository repo = new ProductRepository(tsx.dsl());

            String productsJson = repo.query(repo::getFullQuery, fkQuery);
            int count = repo.count(repo::getFullQuery, fkQuery.getFilters());

            String result = "{ \"products\": " + productsJson + ", \"count\": " + count + " }";

            return jsonMapper.writerWithDefaultPrettyPrinter().writeValueAsString(jsonMapper.readTree(result));
        });
    }

    public Optional<String> getOne(RequestContext requestContext, final Long productId) throws DataAccessException {
        return database1.dsl(requestContext).transactionResult(tsx -> {
            final ProductRepository repo = new ProductRepository(tsx.dsl());
            String result = repo.fetch(repo::getFullQuery, productId);
            if (result == null) {
    ...
    public String create(RequestContext requestContext, final ProductDTO product) throws ValidationException {
        try {
            return database1.dsl(requestContext).transactionResult(tsx -> {
    ...
    public String update(RequestContext requestContext, final ProductDTO product) throws ValidationException {
     ...
     * @return stream
     */
    public Stream<Object> streamAll(RequestContext requestContext) throws InvalidDataException {
        ...
        return database1.dsl(requestContext).transactionResult(tsx -> {
            final ProductRepository repo = new ProductRepository(tsx.dsl());
            Stream<Object> stream1 = repo.stream(repo::getFullQuery, fkQuery);
            Stream<List<Object>> chunkStream = chunk(stream1, 250);
