...
public class ProductRepository extends AbstractRepository<ProductAPI, Long> {
    ...
    public SelectFinalStep<Record1<ProductAPI>> getFullQuery(FkQuery fkQuery) throws InvalidDataException {
            ...
            .select(
                row(
                    ...
                    row(
                        ...
                        ).convertFrom(r -> r.map(RoleAPI::create))
                    ).convertFrom(UserAPI::createOrNull),
                    multiset(
                        select(
                            PRODUCT.product_lang()
                        ).from(PRODUCT.product_lang())
                    ).convertFrom(r -> r.map(ProductLangAPI::create))
                ).convertFrom(ProductAPI::create)
            )
            .from(PRODUCT
